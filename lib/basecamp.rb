...
installed you can install ActiveResource by doing
...
class Basecamp
  ...
  class TodoItem < Resource
    ...
    def self.prefix(options={})
      if options[:todo_list_id]
        "/todo_lists/#{options[:todo_list_id]}/"
      else
        "/"
      end
    end
  ...
  class TimeEntry < Resource
    def self.prefix_source
      "/projects/:project_id/todo_items/:todo_item_id/"
    end

    def self.prefix(options={})
      if options[:todo_item_id]
        "/todo_items/#{options[:todo_item_id]}/"
  ...
  def initialize(url, user_name, password, use_ssl = false)
    ...
    Resource.site = (use_ssl ? "https" : "http") + "://" + url
